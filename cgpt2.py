...
def augment_train_data(model, tokenizer, train_examples,train_label, output_dir,file_name,prefix,max_seq_length,sample_num,temperature,top_k,top_p,repetition_penalty):
    # load the best model
    best_model_path = os.path.join(output_dir, "best_Gpt2.pt")
    ...
    prefix_text = None
    for ex_index, example in tqdm(enumerate(zip(train_examples,train_label))):
        model.eval()
...
def train_cmodgpt2_and_augment(model,tokenizer,train_df,val_df,output='PosGpt2_eda',file_name='augment.tsv',seed = 1234,max_seq_length = 64,sample_num=6):
    ...
    dev_features = convert_examples_to_features(val_examples,val_label,
                                                block_size,
                                                tokenizer, seed)
    ...
    for epoch in trange(int(num_train_epochs), desc="Epoch"):
        ...
        if dev_loss < best_dev_loss:
            ...
            save_model_path = os.path.join(output, 'best_Gpt2.pt')
