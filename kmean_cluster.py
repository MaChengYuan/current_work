...
def umap_plot(model,tokenizer,df,max_len = 40):
    df['output'] = df['sentence'].apply(lambda x : process(model,tokenizer,x,max_len))
    df['predicted'] =  df['output'].apply(lambda x:x[0])
    df['output'] =  df['output'].apply(lambda x:x[1])
    ...
    plt.scatter(X_reduced[:, 0], X_reduced[:, 1], c=y, s=5)
    plt.show()
...
def euclidean(dataframe):
    ...
    for i in df.label.unique():
        ds = df[df.label==i].reset_index(drop=True)
        current_label = len(ds)
        total = 0
        #print(f'current label is {i} and length is {current_label}')
        for j in range(current_label):
            for k in range(j+1,current_label):
    
                total += np.linalg.norm(ds.output[j]-ds.output[k])
        #print(f'current label {i} has Euclidean distance {total}')
        total_Euclidean += total
...
def plot_tsne(model,tokenizer , df , max_len=40):
    ...
    #df = df[df.predicted==df.label]
    #print(df.label.value_counts())
    
    euc_dist = euclidean(df)
    ...
    #kmeans label
    labels = kmeans_analysis_label(df)
    
    return kl_divergence_ , euc_dist , labels
...
def process(model,tokenzier,sentence,max_len):
    ...
    return torch.argmax(output[0], dim=1).flatten(),torch.mean(output[1][0,:,:],0).detach()
