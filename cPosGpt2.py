...
def convert_examples_to_features(examples,examples_labels, block_size, tokenizer, seed=12345):
    ...
    for (ex_index, example) in enumerate(zip(examples,examples_labels)):
        ...
        x = get_postags(tokenizer.tokenize(example[0]))
        prefix_size = 0
        for i in x :
            if(i == 'NN' or i == 'NNS'):
                prefix_size = x.index(i)
                break
        if prefix_size > 0:
        ...
        if ex_index:
            ...
            text += " " + example[1] + SEP_TOKEN + prefix_text + EOS_TOKEN
        else:
            text += example[1] + SEP_TOKEN  + prefix_text + EOS_TOKEN
...
def train_cposgpt2_and_augment(model,tokenizer,train_df,val_df,output='PosGpt2_eda',file_name='augment.tsv',seed = 1234,max_seq_length = 64,sample_num=6):
